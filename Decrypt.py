...
# 2) Взлом зашифрованного русскоязычного текста методом наименьших квадратов на ос- 
# нове идей Казиского (в случае использования идей Фридмана уметь внятно объяснить
# эти идеи) +
...
def remove_non_letters(text: str) -> str:
    """
    Удаляет всё, кроме латинских и кириллических букв.
    """
...
def break_caesar_auto(ciphertext: str):
    rus_shift, rus_score, rus_text = break_for_alphabet(ciphertext, RUS_ALPHABET, RUS_FREQ)
    eng_shift, eng_score, eng_text = break_for_alphabet(ciphertext, ENG_ALPHABET, ENG_FREQ)

    if rus_score < eng_score:
        return {
            "language": "ru",
            "shift": rus_shift,
            "text": rus_text
...
if __name__ == "__main__":
    ...
    print(result)
    # {'language': 'en', 'shift': 3, 'text': 'helloworld'}